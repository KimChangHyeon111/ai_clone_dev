...
class DataLoader:
    ...
    def get_fgi_profile(self, clone_srg_key: str, df_pd_de_tot: pl.DataFrame) -> dict:
        ...
        ml_data = ml_history.select([
            pl.col('dt').alias('구매일자'),
            pl.col('pd_nm_full').alias('구매상품'),
            pl.col('buy_am').alias('구매금액')
        ]).to_dicts()

        return {"ms": ms_data, "ml": ml_data, "promo_info": self.promo_info}
